...
from ..custom_activations import TReLU, Quad, TReLU_with_trainable_bias, TReLU
from ..custom_layers import Normalize, take_top_k, DivisiveNormalization2d
...
class VGGblock_with_dn(nn.Module):
    def __init__(self, in_channel, out_channel):
        super().__init__()
        self.in_channel = in_channel
        self.out_channel = out_channel
        ...
        self.conv = nn.Conv2d(self.in_channel, self.out_channel, kernel_size=3, padding=1)
        self.dn = DivisiveNormalization2d(b_type= "linf", b_size= (5, 5), sigma=0.01, global_suppression=True, alpha=0.3)
        self.relu = nn.ReLU(inplace=True)
...
class NT_VGG(nn.Module):
    def __init__(self, vgg_name="VGG11", gamma=1.):
        super().__init__()
        ...
        self.block1 = VGGblock_with_dn(3, 640)
    ...
    def forward(self, x):
        # o = self.norm(x)
        o = self.block1(x, gamma=self.gamma)
        o = self.mp1(o)
        o = self.block2(o)
        o = self.mp2(o)
        o = self.block3(o)
